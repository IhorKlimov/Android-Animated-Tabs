...
public class SmartTabLayout extends HorizontalScrollView {
    private static final String LOG_TAG = "SmartTabLayout";
    ...
    private int finalCirlceYTranslation;
    ...
    public SmartTabLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        setFillViewport(!tabStrip.isIndicatorAlwaysAtStart());
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (tabStrip.isIndicatorAlwaysInCenter() && tabStrip.getChildCount() > 0) {
            ...
            Log.d(LOG_TAG, "onSizeChanged: " + start + " " + end);
            ViewCompat.setPaddingRelative(this, start, getPaddingTop(), end, getPaddingBottom());
            setClipToPadding(false);
        } else if (tabStrip.isIndicatorAlwaysAtStart() && tabStrip.getChildCount() > 0) {
            View firstTab = tabStrip.getChildAt(0);
            View lastTab = tabStrip.getChildAt(tabStrip.getChildCount() - 1);
            int start = (w - Utils.getMeasuredWidth(firstTab)) / 2 - Utils.getMarginStart(firstTab);
            int end = (w - Utils.getMeasuredWidth(lastTab)) / 2 - Utils.getMarginEnd(lastTab);
            ...
            ViewCompat.setPaddingRelative(this, (int) (Utils.getMeasuredWidth(firstTab) / 2 + dpToPixels(20)), getPaddingTop(), end * 2, getPaddingBottom());
            setClipToPadding(false);

            int newHeight = (int) (dpToPixels(48) * 1.7f);
            int oldHeight = (int) (dpToPixels(48));
            finalCirlceYTranslation = (newHeight / 2 - oldHeight / 2) + oldHeight / 2;
        }
    }
    ...
    private void scrollToTab(int tabIndex, float positionOffset) {
        ...
        if (tabStrip.isIndicatorAlwaysInCenter() || tabStrip.isIndicatorAlwaysAtStart()) {
    ...
    private void animateTab(int position, float fraction) {
        ...
        float scaleX = 1.f + .7f * fraction;
        tab.setScaleX(scaleX);
        tab.setScaleY(scaleX);
        tab.setTranslationY(finalCirlceYTranslation * fraction);
    }
